...
import { getRecommendations as aiRecommend } from "../services/recommendationService.js";

export async function listUsers(req, res, next) {
  try {
    const users = await prisma.user.findMany({
      where: { role: "USER" },
      select: { id: true, name: true, email: true, timezone: true, tags: true, description: true, callType: true, createdAt: true },
      orderBy: { name: "asc" },
    });
    res.json(users);
  } catch (e) {
...
export async function updateMentorMetadata(req, res, next) {
  try {
    ...
    const mentor = await prisma.user.findUnique({ where: { id }, select: { role: true } });
    ...
    const updated = await prisma.user.update({
      ...
      select: { id: true, name: true, email: true, tags: true, description: true, role: true },
    });
    res.json(updated);
  } catch (e) {
    next(e);
...
export async function getUserRequirements(req, res, next) {
  try {
    const { id } = req.params;
    const user = await prisma.user.findUnique({
      where: { id },
      select: { id: true, name: true, email: true, role: true, tags: true, description: true, callType: true },
    });
...
export async function getRecommendations(req, res, next) {
  try {
    ...
    if (!callType) return res.status(400).json({ error: "callType is required" });
    ...
    if (userId) {
      const user = await prisma.user.findUnique({
        where: { id: userId },
        select: { tags: true, description: true, callType: true },
      });
      if (user) {
        userDescription = user.description || "";
        userTags = Array.isArray(user.tags) ? user.tags : [];
        effectiveCallType = callType || user.callType || callType;
      }
    }
    ...
    const recommendations = await aiRecommend({
    ...
    res.json({ recommendations, callType: effectiveCallType });
  } catch (e) {
    next(e);
  }
}
